...
**v2.13.3 — fixed White Noise not playing.** No schema change.
- **Fix**: the White Noise ambient sound shipped in the v2.13.1 and v2.13.2 release bundles as a stale `resources/white_noise.ogg` left over from an older version of `tools/generate_ambient.py` — the app (`ambient_loop.py`) has looked for `white_noise.wav` for a while now, so that sound silently never played from either of those two releases' zips (a from-source checkout with freshly-generated resources was unaffected). This release's bundle has all six ambient tracks regenerated fresh from the current `tools/generate_rain.py` / `tools/generate_ambient.py`.
- **Also in this release**: a real Inno Setup-based `Pawmodoro-Setup.exe` installer (`windows_installer/`, built by `.github/workflows/build-windows-installer.yml`) alongside the usual zip, needed for submitting Pawmodoro to package managers like winget that require a proper silent installer rather than a zip and script. It doesn't change how the zip/in-app updater work at all.
- **Honest testing note**: confirmed by regenerating the resources locally and diffing filenames/sizes against what `ambient_loop.py`'s sound registry expects; not confirmed by actually pressing play on White Noise on a real machine (no audio device in this sandbox). The new installer compiled successfully on a real Windows GitHub Actions runner but hasn't been run/installed on a real Windows machine yet.
...
- **Honest testing note**: the dialog's button, menu entry, link copy and open were driven headless (the browser call stubbed, so nothing was opened); the emoji fix was reproduced and verified headless (`QT_QPA_PLATFORM=offscreen`) on the affected Fedora 44 machine, Qt 6.11: blank before, all icons drawn after, in both the paper and the monospace hacker theme. Windows/macOS unchanged by design but not run.

**v2.13.0 — shared-list notifications.** *(Optional one-time re-run of `supabase/schema.sql` — see MOBILE_SYNC.md.)*
- **Notifications** (`shared_activity.py`, `MainWindow._notify_shared_activity`): a desktop notification (the taskbar / system tray one, like the reminders) when another household member **adds** a shared item ("Sam added: Milk") or **completes** one ("Sam completed: Milk"). A burst is summarised in one message ("Sam added 5 items: …"). Switch it off with **🔔 Notify me when someone else adds or ticks off an item** at the top of the Shared tab (saved in `data.json`).
- **What counts**: the pull is compared with the list cached before it, so only *other* people's changes appear — your own edits are already in the cache. Un-ticking, weekly/daily resets, removals and reordering are silent, as is joining a household or the very first sync (nothing to compare with). Items added while the app was closed are announced when it next syncs.
- **Schema**: `household_pull` now also returns `created_by_name`, `created_by_me` and `done_by_me` per item, so the message can name the person and skip changes made from your own phone. The re-run is optional: on an un-updated schema you still get the notifications, minus the name of whoever added an item, and an item you add on your own phone can notify your desktop.
- **Cleanup**: removed four unused imports.
- **Honest testing note**: the detection rules and the window's behaviour (including refused stale pulls, a different household, the switch off) were tested headless (`QT_QPA_PLATFORM=offscreen`) with a throwaway home directory, and the schema change was loaded twice into a real local Postgres and pulled as two separate users (each sees the right `created_by_me` / `done_by_me`). Not exercised live: the actual tray pop-up on Windows (the same `showMessage` call the reminders already use) and a real second person's device.
