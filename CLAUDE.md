...
**Deckbuilder's own sync** (`deckbuilder_decks`, `deckbuilder_collection`, `deckbuilder_wants`, RPCs `deckbuilder_save_deck` / `deckbuilder_delete_deck` / `deckbuilder_set_collection_quantity` / `deckbuilder_set_for_trade` / `deckbuilder_wishlist_set_quantity` / `deckbuilder_sync_pull`) is unrelated to the rest of this project — it's the Deckbuilder repo's own two-way sync between its desktop app and phone PWA, reusing this shared Supabase project purely because Deckbuilder already asks people to connect their Pawmodoro account (for wishlist push and trading, see `deckbuilder_set_profile` / `deckbuilder_sync_collection` / `deckbuilder_sync_wants` / `deckbuilder_browse` / `deckbuilder_matches` just above it in `schema.sql`, which those RPCs are deliberately left untouched by and still work exactly as before). It mirrors this project's own sync shape (a purely client-side monotonic rev, never stored here, gates whether a `deckbuilder_sync_pull` result gets applied — see `storage.py`'s `adopt_remote_state` for the original pattern) rather than inventing a new one. No tombstone/soft-delete column anywhere: a row that's gone from the next `deckbuilder_sync_pull` result is simply absent from the client's full local replace of that list. See the Deckbuilder repo's own CLAUDE.md-equivalent docs for its client-side half of this. `deckbuilder_list_decks` (decks only, card lists stripped, plus a server-computed `card_count`) is read by a *third* app, Pairings (the TCG tournament tracker, `~/Desktop/pairings`, its own separate Supabase project), whose Decks screen logs in to this project with the user's Brewhouse account to import/re-sync decks; it reads each deck's `data.summary` (`leader`, `colors`, `formatLabel`, written by Brewhouse's `deckSummary.ts`). Keep that function's output shape stable, or Pairings' `normalizeBrewhouseDeck` breaks (it falls back to `deckbuilder_sync_pull` only when the function is missing).
...
The shared project's URL + anon key are baked in so friends only enter email/password: `cloud_defaults.py` (desktop, used by `sync_settings_dialog.py`), `DEFAULT_CONFIG` in `docs/app.js` (PWA), and `src/shared/pawmodoroDefaults.ts` in the separate Deckbuilder repo (`~/Desktop/deckbuilder`, Electron/TS; its Pawmodoro sync lives in `electron/ipc/pawmodoro.ts` + `WishlistPanel.tsx`). Pairings' `index.html` (`BREWHOUSE_SUPABASE_URL` / `BREWHOUSE_ANON_KEY`) holds a fourth copy. Keep all four identical. The key must stay the `anon` role (decode the JWT's `role` claim to check) — never a `service_role` key. The project needs "Allow new users to sign up" on and "Confirm email" off for friends to self-register.
...
- **In-app update** (`update_checker.py` = logic, no Qt; `update_dialog.py` = the dialog; wired up in `MainWindow._schedule_update_checks` / `_check_for_update`): the app asks the GitHub API for the repo's latest release, and if it's newer than `version.VERSION` shows a header button; clicking it downloads the release's `*-windows.zip` asset (the bundle: both installers + app), verifies its size and the asset's `digest` (sha256), unpacks it and runs `install.ps1 -Relaunch` / `install.sh --relaunch` detached, then quits. So **each release must attach a zip whose name ends `-windows.zip` containing `<folder>/install.sh|ps1` and `version.py`**, and the installers' `-Relaunch`/`--relaunch` switch must keep working. Nothing runs without a click. Never run the real `install.sh` to test: it kills any running Pawmodoro; use a throwaway `HOME`, `TMPDIR` (the single-instance socket lives there) and shimmed `pgrep`/`pkill`.
...
`windows_installer/pawmodoro.iss` is a separate, real silently-installable `Pawmodoro-Setup.exe` (Inno Setup), needed for winget and similar package managers that require a proper installer rather than a zip + script — built by `.github/workflows/build-windows-installer.yml` on a `windows-latest` runner (which ships Inno Setup preinstalled), triggered manually or by a `v*` tag push. It deliberately doesn't reimplement `install.ps1`'s setup logic: it stages the app's source into `%LOCALAPPDATA%\Pawmodoro-src` (a scratch directory, never `%LOCALAPPDATA%\Pawmodoro` itself — `install.ps1` wipes its destination before copying from its own script directory, so the two must differ) and runs `install.ps1` from there, exactly like a manual zip extract + `install.bat` double-click. This is a separate release asset from the `*-windows.zip` the in-app updater reads — don't confuse the two, and don't let this one replace or change that one.
